...
struct LexicalAnalyzer {
    ...
    void next() {
        ...
        if (s[cur] == '(')
            ...
            //db2(cur, s.size());
            cerr << "unexpected char " << s[cur] << " position: " << cur << endl;
            exit(1);
            //assert(false);
        }
        cur++;
...
struct Parser {
    ...
    Node * parseTerm() {
        if (lex.getToken() == LPAREN) {
            ...
            //if (lex.getToken() != RPAREN) {
                //if (lex.getToken() == lex.END) {
                    //cerr << "unexpected endof line\n" << endl;
                //}
                //cerr << "found " << lex.s[lex.cur - 1] << " expected ) " << endl;
                //exit(1);
            //}
    ...
    Node * parseANDPrime() {
        if (lex.getToken() == AND) {
            lex.next();
            Node * v = parseTerm();
            Node * u = parseANDPrime();
            return new Node("ANDPrime", 3, new Node("TAND", 0), v, u);
        }
        else {
            auto t = lex.getToken();
            assert(t == XOR || t == OR || t == RPAREN || t == END);
            return new Node("XORPrime", 0);
        }
    }
    ...
    Node * parseXORPrime() {
        if (lex.getToken() == XOR) {
            lex.next();
            Node * v = parseAND();
            Node * u = parseXORPrime();
            return new Node("XORPrime", 3, new Node("TXOR", 0), v, u);
        }
        else {
            auto t = lex.getToken();
    ...
    Node * parseORPrime() {
        if (lex.getToken() == OR) {
            ...
            Node * v = parseXOR();
            Node * u = parseORPrime();
            return new Node("ORPrime", 3, new Node("TOR", 0), v, u);
        }
        else {
            assert(lex.getToken() == RPAREN || lex.getToken() == END);
            return new Node("ORPrime", 0);
        }
    }
